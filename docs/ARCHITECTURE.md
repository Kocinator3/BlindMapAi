...
Line precision and coverage use 160 arc-length samples each direction and a harmonic mean. Areas use the bidirectional RMS boundary distance sampled at 160 arc-length positions, plus half the absolute difference of square-root areas (both in km). The greater error determines proximity after forgiving half the configured tolerance. No overlap percentage determines the score. Both directions discourage tiny guesses and large enclosing areas; narrow shifted outlines remain usable. The existing polygon-area/intersection utilities retain analytical tests. Regional projection limitations remain; the total 500-vertex cap remains, while MultiLineString permits up to 250 two-point components under the same cap.
