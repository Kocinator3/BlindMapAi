...
`MapCanvas` uses a single `desktopDragThreshold` (6 logical pixels). A mouse down records its origin. For Point/Polyline/Polygon/MultiPoint placement, movement below that threshold remains a click candidate; movement at or beyond it switches to continuous viewport panning and clears any draft, so pointer release cannot add geometry. Freehand and vertex-move deliberately keep their drag gestures. Space+left or middle drag temporarily pans even during those tools. Mouse wheel zoom and the existing touch scale/pinch path remain independent. `MouseRegion` reports precise geometry, basic navigation, and grabbing cursors.
