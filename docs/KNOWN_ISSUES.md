...
- Schema v1 intentionally rejects Polygon holes/MultiPolygon and unknown fields. Map layers include country outlines, worldwide rivers, lakes with island holes and all source cities/capitals without labels. Per-question overrides and full viewport controls remain unfinished. Catalog lake questions use generalized exterior rings only (islands are not subtracted in scoring); multipart source objects are separate selectable parts. Coverage is limited to Natural Earth and most names remain in the source language.
...
- Practice and learning share feedback behavior. Results now offer timed replay and up to five weakest locations from the completed run. Challenge has timer/combo but no combo reward. Highscores currently aggregate each session's recorded answers, including partial sessions. Scores are not partitioned by difficulty settings/tolerance, and more complete progress dashboards remain unfinished.
...
## Resolved audits
- Area scoring uses projected polygon intersection (sqrt IoU). Analytical polygon-area/intersection tests cover thin, concave, antimeridian and high-latitude regions.
